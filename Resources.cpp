...
std::string readDoubleQuotedString (std::istream &in)
{
    ...
    std::string errInStream = "ERROR: in readDoubleQuotedString function>>\n";
...
} // END function readDoubleQuotedString 

std::istream& operator>>(std::istream &is, Date &dateIn)
{
    char ch;
    int year = -1;
    int month = -1;
    ...
    try
    {
        ...
        is >> year; //extract the year and store them in local variable
        if (!is) { streamFailed = true; }
        
        is >> ch; //extract and discard the dilimiting character
        if (!is) { streamFailed = true; }
        
        is >> month; //extract the month and store them in local variable
        if (!is) { streamFailed = true; }

        is >> ch; //extract and discard the dilimiting character
        if (!is) { streamFailed = true; }

        is >> day; //extract the day and store them in local variable
        if (!is) { streamFailed = true; }        
        ...
        dateIn.setDate(year, month, day); // call setDate method where validation will take place
...
{
    this->setDate(otherDate.yyyy, otherDate.mm, otherDate.dd);
...
void Date::setDate(int year, int month , int day)  //yyyy, mm, dd
{
    ...
    if ( year < MIN_YYYY || month < MIN_MM || day < MIN_DD || year > MAX_YYYY || month > MAX_MM
                         || day > MAX_DD[month])
    {
        badArguments = true;
...
} // END member function Date::getYear

int Date::getMonth() const
{
    return mm;

} // END member function Date::getMonth

int Date::getDay() const
{
    return dd;

} // END member function Date::getDay

bool Date::operator ==( const Date &rhsObj ) const
{
    bool answer = false;

    answer = ( this->yyyy == rhsObj.yyyy && this->mm == rhsObj.mm && this->dd == rhsObj.dd );
 
    return answer;

} // END overloaded equality operator for Date class
...
} // END overloaded inequality operator for Date class
...
} // END overloaded less than or equal to operator for Date class
...
} // END overloaded less than operator for Date class
...
} // END overloaded greater or equal to operator for Date class
...
} // END overloaded greater than operator for Date class
...
} // END overloaded less than operator, Time class